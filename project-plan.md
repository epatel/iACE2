...
| 4 ✅ | **Keyboard** | `tool/` xib → `keyboard_map.json`, photo keyboard with hit regions, sticky shift, multi-touch shifts, minimum key hold, `PortraitFrame` pillarbox, pixel-perfect screen scaling | You can type FORTH on screen; the key matrix is tested per key. Widget tests type `2 3 + .` through the keys. Real taps via `adb` on the Pixel Tablet emulator typed `123` and `vlist` |
| 5 ✅ | **Persistence** | drift DB v1 + schema dump + migration tests, settings, tapes (SAVE/LOAD via ED FC/FD), Frogger seed, snapshot auto-save/restore, iACE 1.2 legacy import, plist reader | `SAVE`/`LOAD` round-trip; `LOAD frogger` plays; kill and relaunch restores the session. Verified on the iPad simulator: legacy files imported, and a word defined before a terminate still ran after relaunch |
| 6 ✅ | **Audio** | beeper edge log + miniaudio output, volume setting, audio-session handling (interruptions, silent switch policy) | `BEEP` in FORTH is audible on both platforms with no clicks or pops. Measured 625 Hz for `200 1000 BEEP`. The device ran on the host Mac, the iPad simulator and the Android emulator (AAudio). Listening on real hardware is still to do |
| 7 ✅ | **Manual** | pdfrx viewer, page slider, remember page, `annotations.json` overlay with `goto`/`open`/`type` actions (the debug edit mode is not ported; edit the JSON) | Tapping a "type" example in the manual runs it in the emulator. Integration test on the iPad simulator and Android emulator: the PDF loads, the page-10 Enter types `vlist`, links jump, swipe pages |
| 8 ✅ | **Shell and UX** | drawers with drag and snap, settings lid (reset confirm, sticky shift, **volume**, links), reveal hint, icons and splash, portrait lock | Matches original behaviour. Drawer rules are ported from iACE 1.x and unit-tested. Checked on the iPad Pro 13" simulator (closed, open, lid and panel) and the Pixel Tablet (a real tap on the manual opens the drawers) |
| 9 ✅ (code) | **Polish and release** | tape browser with `.TAP` import/export, legacy import from iACE 1.2, accessibility pass, performance profiling, store metadata, GPL notices and source link | Release builds compile (`.apk`/`.aab`, iOS `--no-codesign`). The release APK ran and typed on the Pixel Tablet emulator. Core: 27 µs per 20 ms frame on an M4. **Still to do by hand:** signing, a public source URL, the store checklist in `store/listing.md`, and testing on real tablets |
...
7. **Phones (decided 2026-09-25):** phones are supported in portrait only; iOS is now universal and Android has no screen filter. A window whose shortest side is under 600 gets `PhoneHome`. It has no manual; the settings panel links to `assets/manual.pdf` on GitHub instead. The screen drawer sits at the top and the keyboard photo below it. The settings panel lies under the keyboard, where tablets have the manual, and the keyboard lifts by exactly the panel's height to reveal it. On the first run the keyboard lifts a little once as a hint. Key tap areas grow until they touch their neighbours; the key images are unchanged.
